...
fn normalize_input<T: IntoIterator<Item = String>>(lines: T) -> Vec<Record> {
    ...
    let creditors: BTreeSet<String> = records.iter().map(|rec| rec.creditor.clone()).collect();
    // We need records also for persons with no expenses so they become part of
    // the calculation further down.
    let debtors_with_no_credit = participants.difference(&creditors).map(|debtor| Record {
    ...
    records
        .into_iter()
        .chain(debtors_with_no_credit)
        .map(|record| {
            let debtors: BTreeSet<String> = if record.debtors.is_empty() {
                &participants
            } else {
                &record.debtors
            }
    ...
    I: IntoIterator<Item = &'a Money>,
{
    amounts
        .into_iter()
        .fold(money::zero(), |memo, elem| memo + elem)
}

impl Records {
    fn new<T: IntoIterator<Item = String>>(records_init: T) -> Records {
        Records {
            records: normalize_input(records_init),
        }
    }
    ...
    fn calc_expenses_per_person_and_group(
        ...
        self.records_by_group()
            .iter()
            .map(|(group, records)| (group.clone(), Self::calc_expenses_per_person2(records)))
            .collect()
    }
    ...
    fn expenses_creditor_not_part_of_group(&self) -> BTreeMap<String, Money> {
        self.records
            .iter()
    ...
    fn calc_debt_per_person(&self) -> BTreeMap<String, Money> {
        let share_per_person_and_group = self.calc_share_per_person_and_group();
        let expenses_per_person_group = self.calc_expenses_per_person_and_group();
        share_per_person_and_group
            .into_iter()
            .map(|(group, share_per_person)| {
                ...
                share_per_person
                    ...
                    .map(|(person, share)| {
                        let debt =
                            share - expenses_per_person.get(&person).unwrap_or(&money::zero());
                        (person.clone(), debt)
                    })
            ...
            .fold(BTreeMap::new(), |mut memo, debt_per_person| {
                for (person, debt) in debt_per_person {
                    let debt_acc = memo.entry(person).or_insert_with(money::zero);
    ...
    fn calc_debt_resolution(&self) -> Vec<Debt> {
        ...
        let debt_per_person_by_debt = {
            let mut d: Vec<(String, Money)> = debt_per_person
                .iter()
        ...
        debt_per_person_by_debt
            .into_iter()
            .flat_map(|(person, debt)| {
                self.resolve_for_person(&person, &debt, &mut expense_per_person)
            })
            .collect()
    }
    ...
    fn resolve_for_person(
        ...
        while &debt != &last_debt && &debt != &zero {
            ...
            let pos_opt = expense_per_person
                .iter()
            ...
            if let Some(pos) = pos_opt {
                ...
                if &remainder >= &zero {
                    debt = remainder;
                    payouts.push((creditor.clone(), expense.clone() * money::from(-1)));
                    *expense = money::zero();
...
mod tests {
    ...
    fn test_normalize_input_no_debtors() {
        let actual = normalize_input("a 100\nb 100\n\n".lines().map(|x| x.to_owned()));
        let expected = vec![
    ...
    fn test_calc_share_per_person_and_group() {
        ...
        let expected = vec![
            ...
            (
                str2btree_set("a c"),
                vec![("a".into(), parse("50")), ("c".into(), parse("50"))]
                    .into_iter()
                    .collect(),
            ),
            (
                str2btree_set("b c"),
                vec![("b".into(), parse("100")), ("c".into(), parse("100"))]
                    .into_iter()
                    .collect(),
            ),
        ]
    ...
    fn test_calc_expenses_per_person_and_group() {
        ...
        let expected = vec![
            (
                str2btree_set("a b c"),
                vec![("a".into(), parse("1200")), ("b".into(), parse("600"))]
                    .into_iter()
                    .collect(),
            ),
            (
                str2btree_set("a c"),
                vec![("c".into(), parse("100"))].into_iter().collect(),
            ),
            (
                str2btree_set("b c"),
                vec![("b".into(), parse("200"))].into_iter().collect(),
            ),
        ]
        .into_iter()
        .collect();
